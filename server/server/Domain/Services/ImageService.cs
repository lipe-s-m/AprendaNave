...
public class ImageService : IImageService
{
    ...
    {
        var envUrl = config["CLOUDINARY_URL"]; // O .NET busca no user-secrets automaticamente
    ...
    public async Task<ImageUploadResult> AddImageAsync(IFormFile file, string folderName, string name)
    {
        ...
        if (file.Length > 0)
        {
            ...
            var uploadParams = new ImageUploadParams
            {
                ...
                Folder = folderName, // 👈 Aqui a mágica acontece!
                PublicId = name, // Nome do arquivo sem extensão
                Overwrite = true, // Assim, se ele trocar de foto, a antiga é substituída
