...
export class GerenciarQuizComponent implements OnInit {
  ...
  titulo = ''; descricao = ''; notaMinima = 70; tempoPorQuestaoSegundos: number | null = null; enunciado = ''; explicacao = ''; alternativas = ['', '', '', '']; correta = 0;
  ...
  carregar(): void { this.carregando.set(true); this.quizService.getQuizCriador(this.moduloId).subscribe({ next: ({ quiz }) => { this.quiz.set(quiz); if (quiz) { this.titulo = quiz.titulo; this.descricao = quiz.descricao || ''; this.notaMinima = quiz.notaMinima; this.tempoPorQuestaoSegundos = quiz.tempoPorQuestaoSegundos ?? null; } this.carregando.set(false); }, error: () => { this.toastr.error('Não foi possível carregar o quiz'); this.carregando.set(false); } }); }
  salvarQuiz(): void { if (!this.titulo.trim()) return; this.salvando.set(true); const dados = { titulo: this.titulo, descricao: this.descricao, notaMinima: Number(this.notaMinima), tempoPorQuestaoSegundos: this.tempoPorQuestaoSegundos === null || this.tempoPorQuestaoSegundos === 0 ? null : Number(this.tempoPorQuestaoSegundos) }; const req = this.quiz() ? this.quizService.atualizarQuiz(this.moduloId, dados) : this.quizService.criarQuiz(this.moduloId, dados); req.subscribe({ next: () => { this.toastr.success('Quiz salvo como pendente de aprovação.'); this.salvando.set(false); this.carregar(); }, error: (e) => { this.toastr.error(e.error?.error || 'Erro ao salvar quiz'); this.salvando.set(false); } }); }
