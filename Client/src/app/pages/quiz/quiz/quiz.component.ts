...
export class QuizComponent implements OnInit, OnDestroy {
  ...
  tempoRestante = signal<number | null>(null); private timer: ReturnType<typeof setInterval> | null = null;
  ...
  iniciar(): void { this.iniciando.set(true); this.quizService.iniciarTentativa(this.moduloId).subscribe({ next: (tentativa) => { this.tentativa.set(tentativa); this.indice.set(0); this.respostas.set(new Map()); this.iniciando.set(false); this.iniciarTimer(); }, error: (e) => { this.toastr.error(e.error?.error || 'Não foi possível iniciar o quiz'); this.iniciando.set(false); this.carregarStatus(); } }); }
  questaoAtual() { return this.tentativa()?.questoes[this.indice()]; }
  /** Letra do badge da alternativa (A, B, C, ...) — suporta mais de 26 itens com AA/AB. */
  letra(indice: number): string { const n = indice + 1; let s = ''; let resto = n; while (resto > 0) { resto--; s = String.fromCharCode(65 + (resto % 26)) + s; resto = Math.floor(resto / 26); } return s; }
  ...
  proxima(forcar = false): void { if (!forcar && !this.respostas().has(this.questaoAtual()?.id ?? 0)) return; if (this.indice() < (this.tentativa()?.questoes.length ?? 0) - 1) { this.indice.update((i) => i + 1); this.iniciarTimer(); } else this.finalizar(); }
  anterior(): void { if (this.indice() > 0) { this.indice.update((i) => i - 1); this.iniciarTimer(); } }
  private pararTimer(): void { if (this.timer) clearInterval(this.timer); this.timer = null; this.tempoRestante.set(null); }
  private iniciarTimer(): void { this.pararTimer(); const limite = this.tentativa()?.quiz.tempoPorQuestaoSegundos; if (!limite) return; this.tempoRestante.set(limite); this.timer = setInterval(() => { const tempo = (this.tempoRestante() ?? 1) - 1; this.tempoRestante.set(tempo); if (tempo <= 0) { this.pararTimer(); this.proxima(true); } }, 1000); }
  finalizar(): void { const tentativa = this.tentativa(); if (!tentativa || this.finalizando()) return; this.pararTimer(); this.finalizando.set(true); const respostas = [...this.respostas()].map(([questaoId, alternativaId]) => ({ questaoId, alternativaId })); this.quizService.finalizarTentativa(this.moduloId, tentativa.tentativaId, respostas).subscribe({ next: (resultado) => { this.resultado.set(resultado); this.finalizando.set(false); }, error: (e) => { this.toastr.error(e.error?.error || 'Não foi possível corrigir o quiz'); this.finalizando.set(false); } }); }
