...
export class DesafioEventoComponent implements OnInit, OnDestroy {
  slug = ''; evento: EventoDesafio | null = null; ranking: RankingEventoEntrada[] = []; carregando = true; modoRanking = false;
  ...
  carregar(): void { this.eventoService.getEvento(this.slug).subscribe({ next: (dados) => { this.evento = dados.evento; this.ranking = dados.entradas; this.carregando = false; if (!this.modoRanking) this.prepararParticipacao(); }, error: () => { this.carregando = false; this.toastr.error('Evento não encontrado ou indisponível.'); } }); }
  carregarRanking(): void { if (!this.slug) return; this.eventoService.getRanking(this.slug).subscribe({ next: (dados) => this.ranking = dados.entradas }); }
